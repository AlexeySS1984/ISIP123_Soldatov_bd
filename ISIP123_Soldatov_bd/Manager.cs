...
public class ServiceManager
{
    ...
    {
         //List < Player > player = Core.Context.Player.ToList();/*GetCurrentPlayer()?.Balance ?? 0m*/
    ...
    public bool HandleClient(ClientCar car, bool acceptRepair)
    {
        ...
        if (acceptRepair)
        {
            ...
            else
            {
                ...
                var availableParts = _dbContext.Part.Where(p => p.Quantity > 0 && p.PartName != partName).ToList();
        ...
        if (_carsProcessedSincePurchase >= DELIVERY_THRESHOLD)
        {
            ...
            if (pendingPurchases.Any())
            {
                ...
                foreach (var purchaseGroup in pendingPurchases)
                {
                    ...
                    if (part != null)
                    {
                        part.Quantity += purchaseGroup.Quantity;
                        Console.WriteLine($"  + {purchaseGroup.Quantity} шт. {part.PartName}");
                    }
                }
